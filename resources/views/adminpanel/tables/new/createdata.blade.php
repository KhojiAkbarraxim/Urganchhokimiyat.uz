<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Malumot qo'shish</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/2.1.2/sweetalert.min.js"
        integrity="sha512-AA1Bzp5Q0K1KanKKmvN/4d3IRKVlv9PYgwFPvm32nPO6QS8yH1HO7LbgB1pgiOxPtfeg5zEn2ba64MUcqJx6CA=="
        crossorigin="anonymous" referrerpolicy="no-referrer"></script>
    <script src="https://cdn.ckeditor.com/ckeditor5/36.0.0/classic/ckeditor.js"></script>
</head>
<style>
    .box {
        width: 100%;
        background-color: #f9f9f9;
        border: 1px solid #ccc;
        border-radius: 5px;
        padding: 16px;
        margin: 0 auto;
    }

    .ck-editor__editable[role="textbox"] {
        /* editing area */
        min-height: 300px;
    }

    .ck-file-dialog-button,
    .ck.ck-dropdown .ck-button.ck-dropdown__button:not(.ck-button_with-text) {
        display: none !important;
    }
</style>

<body>
    <div class="container">
        <div class="box">
            <form id="createForm" action="{{ route('newss.store') }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <div class="form-group mb-3">
                    <div class="mb-3">
                        <input type="text" class="form-control" id="title_uz" name="title"
                            placeholder="Mavzu qo'shish">
                    </div>
                    <textarea id="text_uz" name="content" class="form-control rounded-3" placeholder="Text qo'shish..."></textarea>
                </div>
                <div class="form-group mb-3">
                    <div class="mb-3">
                        <input type="text" class="form-control" id="title_eng" name="eng_title"
                            placeholder="Mavzuni ingliz tilida qo'shish">
                    </div>
                    <textarea id="text_eng" name="eng_content" class="form-control" placeholder="Textni ingliz tilida qo'shish..."></textarea>
                </div>
                <div class="form-group mb-3">
                    <div class="mb-3">
                        <input type="text" class="form-control" id="title_ru" name="ru_title"
                            placeholder="Mavzuni rus tilida qo'shish">
                    </div>
                    <textarea id="text_ru" name="ru_content" class="form-control" placeholder="Textni rus tilida qo'shish..."></textarea>
                </div>
                <div class="mb-3">
                    <label for="categoryDataList" class="form-label">Kategoriya qo'shish</label>
                    <select class="form-select" name="category" required>
                        @foreach(\App\Enums\NewsCategoryEnum::cases() as $category)
                        <option value="{{ $category->value }}">{{ $category->value }}</option>
                        @endforeach
                    </select>
                </div>
                <br>
                <div class="mb-3">
                    <h5> Rasm qo'shish </h5> Ruxsat berilgan fayl turlari: <span
                        style="color:green">png,jpg,jpeg</span>
                    {{-- <label class="form-label">Rasm qo'shish</label> --}}
                    <input type="file" class="form-control" name="image_path" style=" z-index: 1;">
                </div>
                <div class="form-group d-flex justify-content-end">
                    <button type="submit" class="btn btn-primary me-2">Qo'shish</button>
                    <a href="{{ route('newss.index') }}"><button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">Chiqish</button></a>
                </div>
            </form>
        </div>
    </div>
    <!-- Include SweetAlert script -->
    <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
    <script>
        // Funktsiya yaratish, CKEditorni alohida maydonlar uchun ishga tushirish
        function initCKEditor(selector) {
            ClassicEditor
                .create(document.querySelector(selector))
                .catch(error => {
                    console.error(error);
                });
        }

    </script>
    @extends('adminpanel.layouts.js.createAlert')
    @section('content')
    @endsection
</body>

</html>
