fetch('https://new.amudarya.uz/api/events/4')
    .then(data => data.json())
    .then(data => {

        for (let i = 0; i < data.Data.data.length; i++) {
            document.querySelector(".chora-tadbir").innerHTML += `
    ...
    .catch(() => {
        document.querySelector('.not__found-choratadbir').style.display = 'block'
    })